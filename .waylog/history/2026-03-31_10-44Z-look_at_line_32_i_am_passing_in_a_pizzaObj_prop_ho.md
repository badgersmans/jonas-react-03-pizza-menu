# look at line 32 i am passing in a pizzaObj prop ho
_Exported on 03/31/2026 at 18:44:36 GMT+8 from OpenAI Codex via WayLog_
...
look at line 32 i am passing in a pizzaObj prop how to do nested destructuring?
...
You’re asking how to destructure the `pizzaObj` prop directly in the component params, and I’m checking the surrounding code in `src/index.js` so I can show it in the exact shape your file is using.
...
At [src/index.js:32](/Users/shawnlaw/Downloads/githubed/pizza-menu/src/index.js#L32), you’re passing a single prop called `pizzaObj`:

```jsx
<Pizza pizzaObj={pizza} />
```

So in `Pizza`, if you want nested destructuring directly in the parameter, do this:
...
Why this works:
- outer `{}` destructures the `props`
- `pizzaObj:` says “take the `pizzaObj` prop”
- inner `{ name, ingredients, photoName, price }` destructures that object

So this:

```js
{ pizzaObj: { name, ingredients, photoName, price } }
```
...
would only work if you passed the props like this instead:
...
Small extra fix: your `key` should go on the `<Pizza />` inside `.map()`, not on the `<div>` inside the component:
...
If you want, I can also show you the cleaner version where we destructure `pizzaObj` inside the function body instead of in the parameter.
