# how to fix this
_Exported on 03/31/2026 at 20:14:37 GMT+8 from OpenAI Codex via WayLog_
...
I’m looking at `src/index.js` around line 58 first so we can see the exact issue in context, then I’ll explain the fix in plain English.
...
I found the break: it’s a JSX syntax bug in the `Pizza` component, not the `pizzaObj` prop itself. I’m patching that line so the component renders, then I’ll explain why your destructuring works.
...
The fix is in. I’m doing one quick sanity check on the project setup so I can tell you whether this was the only blocker or if there’s anything else waiting behind it.
...
The bug was on [src/index.js#L58](/Users/shawnlaw/Downloads/githubed/pizza-menu/src/index.js#L58).
...
That breaks because JSX expects one valid expression after the `:` in the ternary. `` `RM` {price} `` is actually two separate things smashed together, so React/JS throws a syntax error.
...
Why this works:
- `` `RM ${price}` `` is one template literal
- `${price}` injects the value into the string
- so if `price` is `12`, React renders `RM 12`

Your prop passing is actually fine. Here:

```jsx
<Pizza pizzaObj={pizza} key={pizza.name} />
```

you pass an object prop called `pizzaObj`.
...
you are destructuring it right in the parameter list. That means:
- grab the `pizzaObj` prop
- then immediately pull out `name`, `ingredients`, `photoName`, `price`, and `soldOut`
...
I already patched it for you in [src/index.js#L58](/Users/shawnlaw/Downloads/githubed/pizza-menu/src/index.js#L58). If you want, I can also rewrite that `Pizza` function in the longer beginner-friendly style so it’s easier to read while you’re learning.
